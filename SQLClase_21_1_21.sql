...
on primary
(name=CorpData, filename = 'C:\Data\Corp1.mdf',size=10MB,filegrowth=25%)
Log on
(name=CorpLog, filename = 'C:\Data\Corp1.ldf',size=5MB,filegrowth=25%)
Go
--- Crear grupo adicional
Alter database Corporacion
add file
(name=CorpData2, filename = 'C:\Data\Corp2.ndf',size=10MB,filegrowth=25%) to filegroup [primary]
Go

--- Grupo de archivos
Alter database Corporacion
add filegroup Division1
go

--- Asociar un archivo
Alter database Corporacion
add file
(name=CorpData3, filename = 'C:\Data\Corp3.ndf',size=10MB,filegrowth=25%) to filegroup [Division1]
Go

--- Grupo de archivos
Alter database Corporacion
add filegroup Division2
go

--- Asociar un archivo
Alter database Corporacion
add file
(name=CorpData4, filename = 'C:\Data\Corp4.ndf',size=10MB,filegrowth=25%) to filegroup [Division2]
Go

--- Grupo de archivos
Alter database Corporacion
add filegroup Division3
go

--- Asociar un archivo
Alter database Corporacion
add file
(name=CorpData5, filename = 'C:\Data\Corp5.ndf',size=10MB,filegrowth=25%) to filegroup [Division3]
Go

--- agregar un file adicional al log
Alter database Corporacion
add log file
(name=CorpLog2, filename = 'C:\Data\CorpLog2.ldf',size=10MB,filegrowth=25%)
Go
...
on [Division3] -- Con este ultimo paso se le dice en que disco (division) guardar esta tabla


--Crear tablas particionadas
/*Se debe crear una funcion que indique los valores de las divisiones de los datos 
	Y luego un esquema que indique los filegroups a donde se destinan los datos*/

Create partition function funciondeparticion (nchar(5))
as range left for values ('i','p')
GO

-- se crea el esquema para las particiones
Create partition scheme schemepartition as partition funciondeparticion
to (Division1, Division2, Division3)
Go

-- Se crea tabla costumer y se le indica la particion 
CREATE TABLE Customers (
	CustomerID nchar (5) NOT NULL ,
...
----------------------
-- Query para revisar los datos en las particiones y en que particiones estan 
Select CustomerId, Companyname, country,
$partition.funciondeparticion(Companyname) as partition
from customers
Go

-- revisar el estado de las opciones de DB
Select DATABASEPROPERTYEX('Corporacion','IsAutoShrink')
go

-- En la consulta anterior aparecia en 0 lo que indica que la funcion autoShrink estaba 
-- Apagado y se procede a encender 
Alter database Corporacion
set AUTO_SHRINK on
Go

-- Informacion de la base de datos se puede utilizar el procedimiento 
sp_helpdb Corporacion

-- Informacion de una tabla en especifico
sp_help customers
...
-- Desvincular una base de datos y luego vincular a otra ubicación
-- Desviculando
use master
Go
...
-- vincular desde otra ubicacion
sp_attach_db 'Northwind',
'C:\Data\northwnd.mdf'
Go

-- El cliente y sus facturas emitidas 
-- Join Interno 
...
-- Cross Join / Plano cartesiano que quiere decir busca relacion de todos contra todos 
Select c.CustomerId, c.CompanyName, c.Country,
o.OrderId, o.OrderDate
...
-- Self Join/ Join a la misma tabla
Select J.EmployeeID, J.firstname + ' ' + J.lastname as Jefe, 
